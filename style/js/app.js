...
const API_BASE_URL = window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1" 
    ? "http://localhost:8000" 
    : ""; // Vacío significa que usará el mismo dominio actual de Vercel
...
async function predictML(event) {
    ...
    const data = {
        presupuesto_usd: parseFloat(
            document.getElementById("ml-presupuesto").value
        ),
        deadline_dias: parseInt(
            document.getElementById("ml-deadline").value
        ),
        tipo_servicio: document.getElementById("ml-servicio").value,
        infraestructura_cloud: parseInt(
    ...
    try {
        ...
        document.getElementById(
            "res-ml-horas"
        ).innerText = `${result.horas_hombre} hrs`;

        document.getElementById(
            "res-ml-costo"
        ).innerText = `$${Number(result.costo_usd).toLocaleString(
            "en-US",
            {
...
async function predictNN(event) {
    ...
    const button = document.getElementById("btn-predict-nn");
    const resultBox = document.getElementById("result-nn");

    // Obtener datos del formulario
    const data = {
        presupuesto_estimado: parseFloat(
            document.getElementById("nn-presupuesto").value
        ),
        deadline_dias: parseInt(
            document.getElementById("nn-deadline").value
        ),
        cantidad_bugs: parseInt(
            document.getElementById("nn-bugs").value
        ),
        numero_desarrolladores: parseInt(
            document.getElementById("nn-devs").value
        ),
        infraestructura_nube_previa: document.getElementById("nn-infra").value,
        renovacion_contrato: document.getElementById("nn-renovacion").value,
    ...
    try {
        ...
        document.getElementById(
            "res-nn-costo"
        ).innerText = `S/ ${Number(result.costo_pen).toLocaleString(
            "es-PE",
            {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2
